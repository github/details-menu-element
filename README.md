...
- `details-menu-select` (cancelable) - fired on `<details-menu>` with `event.relatedTarget` being the item to be select.
- `details-menu-selected` - fired on `<details-menu>` with `event.relatedTarget` being the item selected, after label is updated and menu is closed.
